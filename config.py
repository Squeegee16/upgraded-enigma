...
class Config:
    ...
    # -------------------------------------------------------
    USE_MOCK_DEVICES = os.environ.get(
    ...
    # -------------------------------------------------------
    CALLSIGN_FILE = os.environ.get(
        'CALLSIGN_FILE',
        os.path.join(BASE_DIR, 'data', 'callsigns', 'callsigns.txt')
    )
    CALLSIGN_DB_URL = 'https://apc-cap.ic.gc.ca/datafiles/amateur_delim.zip'
    CALLSIGN_DB_DIR = os.path.join(BASE_DIR, 'data', 'callsign_db')

    # Single definition of VALIDATE_CALLSIGNS
    ...
    SSL_CERT = os.environ.get(
        'SSL_CERT',
        os.path.join(BASE_DIR, 'data', 'certs', 'cert.pem')
    )
    SSL_KEY = os.environ.get(
        'SSL_KEY',
        os.path.join(BASE_DIR, 'data', 'certs', 'key.pem')
    )
    ...
    # Auto-detected by entrypoint script, or override here
    # -------------------------------------------------------
    ...
    GPS_SERIAL_PORT = os.environ.get('GPS_SERIAL_PORT', '/dev/ttyAMA0')
    ...
    GRID_PRECISION = int(os.environ.get('GRID_PRECISION', '6'))
