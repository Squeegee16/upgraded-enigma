...
# Disable gpsd from auto-starting and claiming serial ports
...
# Remove any apt-installed Hamlib before building from source
# This prevents version conflicts and ensures rigctld 4.7.0 is used
RUN apt-get update && \
    apt-get remove -y --purge libhamlib* hamlib* 2>/dev/null || true && \
...
RUN groupadd -r hamradio -g 1000 && \
    useradd -r -g hamradio -u 1000 -m -s /bin/bash \
...
CMD ["python", "app.py"]# Ham Radio Operator Web Application - Dockerfile
# =============================================
# Multi-stage build for optimized image size and security.
#
# Stage 1 (builder): Installs all Python dependencies into a virtual environment.
